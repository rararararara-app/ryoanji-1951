...
// Sparse drifting petals and leaves, mostly outside and in the court. A stylization, not evidence of season.
const VOLUMES = [
...
const COLORS = ['#f2c9cf', '#f6dedf', '#eab8c0', '#f2c9cf', '#c99a4c', '#98a04e'];

export class Petals {
  constructor(count) {
    const geo = new THREE.PlaneGeometry(0.038, 0.026);
    geo.translate(0.004, 0, 0);
    const mat = withAtmos(new THREE.MeshStandardMaterial({ roughness: 0.9, metalness: 0, side: THREE.DoubleSide, emissive: '#3a2c2c' }));
  ...
  blocked(p) {
    ...
    return p.distanceToSquared(LENS) < 1.2;
  }

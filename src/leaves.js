...
const VOLUMES = [
  { w: 0.6, min: new THREE.Vector3(-2, GROUND, 1.7), max: new THREE.Vector3(12.6, 3.6, 5.2) },  // garden in front, inside the slab
...
export class DriftingLeaves {
  ...
  update(dt, t, pov) {
    ...
    for (let i = 0; i < this.p.length; i++) {
      ...
      if (s.pos.y < GROUND + 0.02 || !inSlab(s.pos.x, s.pos.z, 0.1) || this.blocked(s.pos)) this.spawn(s, false);   // nothing beyond the slab edge
